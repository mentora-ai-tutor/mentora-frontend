...
|-------|------|-------------|
| `/learning-generator` | Overview Dashboard | Hero card with gap/material counts, `ActiveJobsList` (live 5s polling), `ProgressStatsCards`, `KnowledgeGapCard` list + **`MaterialCard` grid of concept-graph-injected prerequisite materials** (shown when any material has `generation_source === "implicit_prerequisite"`), sidebar (`QuickActions`, `ConceptCoverage`, `ModuleProgressList`, `ScoreHistory`, `StrengthsList`). Materials are generated straight from the student's real Knowledge Assist mastery gaps via `submitProfile()` (no hardcoded/mock profile) |
